<?xml version="1.0"?>
<!--+
    | Generate lex files from XML (at the moment only for Komi, Erzya,
    | Moksha, Hill Mari, Veps, Livvi)
    | Applying to Viena Karelian
...
<!-- 	    <xsl:when test="doc-available(.)"> -->
		...
		<out>	
		  ...
		  <xsl:for-each select="./r/entry[not(./@exclude='fst')]/lg/stg/st">
			...
			<xsl:value-of select="normalize-space(@krlContlex)"/>
		      </xsl:attribute>
